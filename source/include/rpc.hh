...
template<class Database, class Datamodel>
std::unique_ptr<JSONRPC::Response> BatchCall::complete(Database &db) {
    ...
    m_req->foreach([&, this](JSONRPC::SingleRequest &single_request){
        try {
            rapidjson::Value single_result =
                SingleCall(&single_request, m_session).complete_call<Database,
                                  Datamodel>(db, batch_response->allocator());
...
rapidjson::Value SingleCall::complete_datamodel_call(
        Database &db, rapidjson::Document::AllocatorType &alloc) {
...
using Impl = rapidjson::Value (Mixin::*)(Database &,
                 SingleCall &, rapidjson::Document::AllocatorType &);
...
public:
    ...
    rapidjson::Value operator()(Mixin &object, Database &db,
           SingleCall &call, rapidjson::Document::AllocatorType &alloc) const {
...
class MethodRoster {
public:
    rapidjson::Value rpc_call(Database &db, SingleCall &call,
                 rapidjson::Document::AllocatorType &alloc) {
